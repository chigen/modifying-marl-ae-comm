...
class Worker(mp.Process):
    ...
    def get_trajectory(self, hidden_state, state_var, done):
        ...
        while not check_done(done) and len(trajectory[0]) < self.t_max:
            ...
            plogit, value, hidden_state, comm_out, comm_ae_loss, traj_comm_out,\
                 traj_comm_ae_loss, img, traj, comm_decoded, traj_comm_decoded = self.net(state_var, self.env, hidden_state)
            ...
            if self.record_comm:
                trajectory[0].append((plogit, action, value, reward, comm_ae_loss,
                                    traj_comm_ae_loss, img, comm_out, comm_decoded, traj, traj_comm_out, traj_comm_decoded))
            else:
                trajectory[0].append((plogit, action, value, reward, comm_ae_loss,
                                    traj_comm_ae_loss))
    ...
    def run(self):
        ...
        while not self.master.is_done():
            ...
            loss = 0
            comm_encode, traj_comm_encode = None, None
            comm_out, comm_decoded, traj_comm_out, traj_comm_decoded = None, None, None, None
            for aid in range(self.num_acts):
                traj = trajectory[aid]
                val = values[aid]
                ...
                for agent in self.agents:
                    ...
                    pls, vls, els = [], [], []
                    # NOTE record comm_out and traj_comm_out here
                    for i, (pi_logit, action, value, reward, comm_ae_loss, 
                            traj_comm_ae_loss, comm_encodei, comm_outi, comm_decodedi, traj_comm_encodei, traj_comm_outi, traj_comm_decodedi) in enumerate(traj):
                    # for i, (pi_logit, action, value, reward, comm_ae_loss, 
                    #         traj_comm_ae_loss) in enumerate(traj):
                        
                        # for j in range(self.env.num_agents):
                        #     self.master.record_comm_info(j, comm_out[j], comm_decoded[j], traj_comm_out[j], traj_comm_decoded[j])
                        comm_encode, traj_comm_encode = comm_encodei, traj_comm_encodei
                        comm_out, comm_decoded = comm_outi, comm_decodedi
                        traj_comm_out, traj_comm_decoded = traj_comm_outi, traj_comm_decodedi
            ...
            self.master.increment(progress_str, 
                                  comm_encode, comm_out, comm_decoded, 
                                  traj_comm_encode, traj_comm_out, traj_comm_decoded)
